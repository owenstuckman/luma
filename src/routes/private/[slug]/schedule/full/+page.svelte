<script lang="ts">
	...
	$: interviewerEmails = [
		...new Set(interviews.map((iv) => iv.interviewer).filter(Boolean))
	].sort() as string[];
	...
	function buildCalendar(filtered: Interview[]) {
		const events = filtered.map((iv) => {
			...
			const violationSummary = isFlagged ? iv.violations!.map((v) => v.detail).join('; ') : '';
			return {
				id: String(iv.id),
	...
	function applyFilter(jobId?: number | null, interviewer?: string, flaggedOnly?: boolean) {
		...
		if (interviewer && interviewer !== 'all')
			filtered = filtered.filter((iv) => iv.interviewer === interviewer);
		...
		for (const iv of interviews) {
			...
			if (newStart_ < ivEnd && newEnd_ > ivStart) {
				if (iv.interviewer === newInterviewerEmail) {
					warnings.push(
						`Interviewer ${iv.interviewer} already has an interview at this time (${iv.applicant || 'unknown applicant'}).`
					);
				}
				if (iv.applicant === newApplicantEmail) {
					warnings.push(
						`Applicant ${iv.applicant} already has an interview at this time (with ${iv.interviewer || 'unknown interviewer'}).`
					);
				}
			}
		}
	...
	async function handleCreate() {
		...
		if (new Date(startISO) < new Date()) {
			createError =
				'Cannot schedule an interview in the past. Please select a future date and time.';
			return;
		}
			...
			{:else}
				...
				<div class="form-row">
					...
					<input
						...
						placeholder="e.g. MCB230, Zoom link, etc."
					/>
				</div>
				...
				<div class="modal-actions">
					...
					<button class="btn btn-quaternary btn-sm" on:click={() => (showModal = false)}
						>Cancel</button
					>
				</div>
			{/if}
