<script lang="ts">
	...
	$: pagedApplicants = filteredApplicants.slice(
		currentPage * PAGE_SIZE,
		(currentPage + 1) * PAGE_SIZE
	);

	$: allSelected =
		pagedApplicants.length > 0 && pagedApplicants.every((a) => selectedIds.has(a.id));
			...
			<div class="applicant-grid">
				{#each pagedApplicants as applicant}
					<div
						...
						on:click={() =>
							selectMode ? toggleSelect(applicant.id) : navigateToReview(applicant.id)}
						on:keydown={() => {}}
						role="button"
						tabindex="0"
					>
			...
			{#if totalPages > 1}
				<div class="pagination">
					...
					<span class="page-info"
						>Page {currentPage + 1} of {totalPages} ({filteredApplicants.length} total)</span
					>
					<button
						class="btn btn-quaternary btn-sm"
