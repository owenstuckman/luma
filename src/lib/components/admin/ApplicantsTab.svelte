<script lang="ts">
	...
	const filteredApplicants = $derived(
		applicants.filter((a) => {
			...
			const orgMatch =
				!appOrgFilter || a.org_name?.toLowerCase().includes(appOrgFilter.toLowerCase());
	...
	const allSelected = $derived(
		filteredApplicants.length > 0 && filteredApplicants.every((a) => selectedApplicantIds.has(a.id))
	);
...
{#if selectedApplicantIds.size > 0}
	<div class="bulk-bar">
		...
		<button class="btn btn-quaternary btn-sm" onclick={() => (selectedApplicantIds = new Set())}
			>Clear</button
		>
	</div>
{/if}
...
<div class="jobs-table">
	...
	{#each filteredApplicants as app}
		<div class="app-table-row table-row" class:row-selected={selectedApplicantIds.has(app.id)}>
			<span class="col-check">
				<input
					type="checkbox"
					checked={selectedApplicantIds.has(app.id)}
					onchange={() => toggleApplicantSelect(app.id)}
				/>
			</span>
			<span class="col-name">
				<button
					class="app-name-btn"
					onclick={() => (expandedApplicantId = expandedApplicantId === app.id ? null : app.id)}
				>
			...
			<span class="col-date"
				><span class="row-sub">{new Date(app.created_at).toLocaleDateString()}</span></span
			>
		</div>
		{#if expandedApplicantId === app.id}
			<div class="app-detail-panel">
				...
				{#if app.org_slug}
					<a
						href="/private/{app.org_slug}/review/candidate?id={app.id}"
...
{#if selectedApplicantIds.size === 0}
	<div style="margin-top: 12px;">
		<button class="btn btn-quaternary btn-sm" onclick={exportApplicantsCsv}
