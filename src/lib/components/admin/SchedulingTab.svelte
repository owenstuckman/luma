<script lang="ts">
	...
	async function applySchedule() {
		...
		try {
			...
			await upsertSchedulingConfig(
				schedOrgId!,
				schedAlgorithmId,
				schedConfig,
				schedJobId || undefined
			);
	...
	async function openEmailModal() {
		...
		try {
			...
			const [interviews, applicants, orgMembers, jobs] = await Promise.all([
				getInterviewsByOrg(schedOrgId),
				getAllApplicants(schedOrgId),
				getOrgMembersWithEmail(schedOrgId),
				getActiveRoles(schedOrgId)
			]);
			onOpenEmailModal({
				interviews,
				applicants,
	...
	{#if schedOrgId}
		...
		{#if schedAlgorithmId === 'batch-scheduler'}
			...
			<div class="form-row">
				...
				{/each}
				<button class="btn btn-quaternary btn-sm" onclick={addRound} style="margin-top: 8px;"
					>+ Add Round</button
				>
			</div>
			...
			<div class="config-grid">
				...
				<div class="form-row" style="grid-column: span 2;">
					...
					<p class="form-hint" style="margin-top: 4px;">
						A second pass places applicants who couldn't be strictly scheduled. These are saved with
						a violation flag for review.
					</p>
				</div>
				...
				<div class="form-row">
					...
					{#each batchAttrRules as rule, i}
						<div class="attr-rule-row">
							...
							<button class="btn-icon-sm" onclick={() => removeAttrRule(i)} aria-label="Remove rule"
								>×</button
							>
						</div>
					{/each}
					...
					<div class="attr-rule-add">
						...
						<input
							...
							placeholder="Member attribute key"
							style="flex: 1;"
						/>
						<input
							type="number"
...
{#if schedOrgId}
	<div class="sched-actions">
		...
		<button class="btn btn-primary" onclick={openEmailModal} disabled={schedEmailLoading}>
			<i class="fi fi-br-paper-plane" aria-hidden="true"></i>
...
<style lang="scss">
	...
	.sched-table-row {
		display: grid;
		grid-template-columns: 1.5fr 1.5fr 100px 130px 120px 80px;
		padding: 10px 16px;
		font-size: 12px;
		align-items: center;
	}
	...
	.rst-row {
		display: grid;
		grid-template-columns: 1.5fr 80px 80px 80px 100px;
		padding: 8px 14px;
		font-size: 12px;
		align-items: center;
	}
